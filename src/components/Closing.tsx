...
export default function Closing({ contactFirstName, slug, easterEgg }: Props) {
  const isPersonalized = contactFirstName !== "Geschätzter";
    ...
    <section className="py-28 lg:py-36 relative overflow-hidden">
      ...
      <div className="mx-auto max-w-3xl px-6 relative z-10">
            ...
            <div className="flex items-center justify-center gap-4 sm:gap-6 flex-wrap">
              {painCount > 0 && (
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-semibold">{painCount} Problemstelle{painCount !== 1 ? "n" : ""}</p>
                    <p className="text-text-muted text-[11px]">in {stageCount} Schritt{stageCount !== 1 ? "en" : ""}</p>
                  </div>
                </div>
              )}
              {fieldCount > 0 && (
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-semibold">{fieldCount} Feld{fieldCount !== 1 ? "er" : ""} erfasst</p>
                    <p className="text-text-muted text-[11px]">Betriebsüberblick</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <ScrollReveal className="text-center">
          ...
          >
            {isPersonalized
              ? `Wir freuen uns auf die nächsten Schritte, ${contactFirstName}.`
              : "Wir freuen uns auf die nächsten Schritte."}
          </p>
          {hasSummary && (
            <p className="text-text-muted text-sm mt-2">
              Alles hier Erfasste ist gespeichert und bereit zur Auswertung.
            </p>
          )}
          <div
